...
def fill_infos_by_ticker(ticker, opener):
    ...
    if (len(last_divYields[:5]) > 0):
        infos[ticker]['media'] = statistics.mean(
            last_divYields[:5])  # https://docs.python.org/3/library/statistics.html#statistics.mean
        infos[ticker]['mediana'] = statistics.median(
            last_divYields[:5])  # https://docs.python.org/3/library/statistics.html#statistics.median

    if (len(last_dpas[:5]) > 0):
        infos[ticker]['constante'] = all(last_dpas[:5][i] > 0 for i in range(len(last_dpas[:5])))
        infos[ticker]['crescente'] = all(
            last_dpas[:5][i] >= last_dpas[:5][i + 1] for i in range(len(last_dpas[:5]) - 1))

    if (len(last_divYields[:5]) > 0):
        infos[ticker]['healthy_payout'] = all(
            (last_payouts[:5][i] > 0) & (last_payouts[:5][i] < 1) for i in range(len(last_payouts[:5])))
...
def fill_score_explanation(shares):
    ...
    shares['Dívida Bruta/Patrimônio < 0.5'] = (shares['Dívida Bruta/Patrimônio']).astype(
        float) < 0.5  # https://www.investimentonabolsa.com/2015/07/saiba-analisar-divida-das-empresas.html https://www.sunoresearch.com.br/artigos/5-indicadores-para-avaliar-solidez-de-uma-empresa/
...
def reorder_columns(shares):
    columns = ['Ranking (Bazin)', 'Cotação', 'Preço Justo (Bazin)', 'Bazin Score', 'Setor', 'Subsetor', 'Segmento',
               'Preço Justo (Bazin) / Cotação', 'Dividend Yield', 'Media de DY (Últimos 5 anos)',
               'Mediana de DY (Últimos 5 anos)']
